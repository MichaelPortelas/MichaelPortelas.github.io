...
function getProduct(productId, productCartColor, productCartQuantity){
    ...
    fetch(`http://live-demo-kanap.portelas.fr:4000/api/products/${productId}`)
        .then(data => data.json())
...
function controlOrder(e){
    ...
    if(listProductId().length != 0){
        ...
        window.location.assign(`http://live-demo-kanap.portelas.fr/front/html/index.html`);
    }
...
function postOrder(postRequest){
    ...
    fetch("http://live-demo-kanap.portelas.fr:4000/api/products/order", {
        method: "POST",
        headers: { 
    ...
    .then(function(value) {        
        ...
        window.location.assign(`http://live-demo-kanap.portelas.fr/front/html/confirmation.html?orderId=${value.orderId}`);
    })
